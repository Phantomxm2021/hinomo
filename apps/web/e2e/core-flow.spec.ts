...
test('owner creates, finds, labels, and maintains a public box', async ({ browser, page }, testInfo) => {
  ...
  await searchbox.fill('冬季')
  await expect(page).toHaveURL(/\/app\/boxes\?q=%E5%86%AC%E5%AD%A3$/)
  await expect(winterLink).toBeVisible()
  ...
  await expect(searchbox).toHaveValue('冬季')
  await expect(page.getByRole('link', { name: '打开冬季衣物' })).toBeVisible()
  await expect(page.getByRole('link', { name: '打开露营用品' })).toHaveCount(0)

  const managementTrigger = page.getByRole('button', { name: '管理冬季衣物' })
  await managementTrigger.click()
  const editLink = page.getByRole('link', { name: '编辑冬季衣物' })
  await expect(editLink).toHaveAttribute('href', /\/app\/boxes\/[^/]+\/edit$/)
