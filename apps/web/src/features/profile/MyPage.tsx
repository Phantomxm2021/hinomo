        ...
        <>
          ...
          <section className="grid gap-5 rounded-shell border border-line bg-surface p-5 shadow-soft sm:p-6" aria-label="账户信息">
            <div className="grid justify-items-center gap-2">
              ...
              {avatarMutation.isError ? <p className="m-0 text-danger" role="alert">头像上传失败，请重试</p> : null}
            </div>
          ...
          <section className="grid gap-4 rounded-shell border border-line bg-surface p-5 shadow-soft sm:p-6" aria-labelledby="preferences-title">
            ...
            {localeMutation.isSuccess ? <p className="m-0 text-sm text-muted" role="status">设置已保存</p> : null}
            {localeMutation.isError ? <p className="m-0 text-danger" role="alert">设置保存失败，请重试</p> : null}
          </section>
