...
test('offers the USD one-time credit packs without a subscription', async () => {
  ...
  expect(screen.getByText('永久解锁无限箱子')).toBeInTheDocument()
  expect(screen.getByText('一次性付款')).toBeInTheDocument()
  expect(screen.queryByText('不限箱子，永久解锁')).not.toBeInTheDocument()
  expect(screen.queryByText('一次付费 · 不自动续费')).not.toBeInTheDocument()
