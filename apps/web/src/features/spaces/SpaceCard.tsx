    ...
    <article className="group flex min-w-0 flex-col overflow-hidden rounded-card border border-line bg-surface">
      <Link className="min-w-0 flex-1 text-muted no-underline" to={`/app/boxes?space=${encodeURIComponent(space.id)}`}>
        <span className={`flex min-h-28 items-start justify-between p-5 ${spaceTone(index)}`}>
          <span className="text-4xl leading-none" role="img" aria-label={t('spaces.iconAlt', { name: space.name })}>{spaceEmoji(space.name)}</span>
          <span className="rounded-full bg-surface/75 px-3 py-1 text-meta font-bold text-ink">{t('spaces.boxCount', { count: space.box_count })}</span>
        </span>
        <span className="block p-5">
          ...
          <p className="min-h-6 truncate text-meta text-muted">{space.description || t('spaces.descriptionUnset')}</p>
          <strong className="mt-4 block text-body font-bold text-ink">{t('spaces.itemCount', { count: space.item_count })}</strong>
        </span>
      </Link>
