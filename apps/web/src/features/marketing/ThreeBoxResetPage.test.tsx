...
it('renders the English three-box reset conversion path', () => {
  ...
  expect(demoVideo).not.toBeNull()
  expect(demoVideo).toHaveClass('h-full', 'w-full', 'object-cover')
  expect(demoVideo?.parentElement?.parentElement).toHaveClass('aspect-video')
  expect(within(demo).getByTestId('campaign-video-primary-control')).toBeVisible()
  expect(demoVideo).not.toHaveAttribute('controls')
