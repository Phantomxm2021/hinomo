...
export function VenueFilterBar({ venues, selectedId, onSelect, onCreate, onEdit }: {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2" aria-label="场地筛选">
      <div className="venue-filter-scroll flex min-w-0 flex-1 gap-2 overflow-x-auto overscroll-x-contain" role="group" aria-label="选择场地">
        ...
        {venues.map((venue) => (
          <span className={`inline-flex shrink-0 items-center rounded-full ${selectedId === venue.id ? 'bg-brand text-white' : 'bg-surface text-muted shadow-[inset_0_0_0_1px_var(--color-line)]'}`} key={venue.id}>
            <button className="min-h-11 rounded-l-full px-4 text-sm font-semibold text-inherit" type="button" aria-label={`${venue.name}，${venue.space_count} 个空间`} aria-pressed={selectedId === venue.id} onClick={() => onSelect(venue.id)}>
              {venue.name}<span className="ml-1 opacity-70">{venue.space_count}</span>
            </button>
            <button className="grid size-11 place-items-center rounded-r-full text-inherit opacity-75 hover:bg-black/5 hover:opacity-100" type="button" aria-label={`重命名场地${venue.name}`} title={`重命名${venue.name}`} onClick={() => onEdit(venue)}>
              <AppIcon name="edit" size={16} />
            </button>
          </span>
        ))}
      </div>
      <button className="grid size-11 shrink-0 place-items-center rounded-full bg-brand/12 text-brand hover:bg-brand/20" type="button" aria-label="创建场地" title="创建场地" onClick={onCreate}>
