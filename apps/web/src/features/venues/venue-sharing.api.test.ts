  ...
  it('calls each member and invite RPC with the typed parameter names', async () => {
    mockRpc.mockImplementation((name: string) => {
      const rows = {
        ...
        create_venue_invite: [{ invite_id: 'invite-1', token: 'secret', expires_at: '2026-08-10T00:00:00Z', reusable: true, ignored: 'must-not-leak' }],
        ...
        list_venue_invites: [{ invite_id: 'invite-1', created_at: '2026-08-09T00:00:00Z', expires_at: '2026-08-10T00:00:00Z', status: 'active', reusable: true, accepted_count: 2, ignored: 'must-not-leak' }],
      } as Record<string, unknown>
      return Promise.resolve({ data: rows[name] ?? [], error: null })
    })

    await expect(listVenueMembers('venue-1')).resolves.toHaveLength(1)
    await expect(createVenueInvite('venue-1')).resolves.toEqual({ invite_id: 'invite-1', token: 'secret', expires_at: '2026-08-10T00:00:00Z', reusable: true })
    ...
    await expect(listVenueInvites('venue-1')).resolves.toEqual([{
      invite_id: 'invite-1', created_at: '2026-08-09T00:00:00Z', expires_at: '2026-08-10T00:00:00Z', status: 'active', reusable: true, accepted_count: 2,
    }])
