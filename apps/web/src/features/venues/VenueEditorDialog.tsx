      ...
      <section className="w-full max-w-lg rounded-shell border border-line bg-surface p-6 shadow-float" role="dialog" aria-modal="true" aria-busy={pending} aria-labelledby="venue-editor-title">
        ...
        <form className="grid gap-3" onSubmit={(event) => void submit(event)}>
          ...
          <div className="mt-3 flex flex-wrap justify-end gap-2">
            {venue ? <button className="mr-auto min-h-11 rounded-control px-4 font-bold text-danger disabled:opacity-50" type="button" disabled={pending || venue.space_count > 0} onClick={() => void onDelete(venue)}>删除场地</button> : null}
            ...
            <button className="min-h-11 rounded-control bg-brand px-5 font-bold text-white" type="submit" disabled={pending}>{pending ? '保存中…' : venue ? '保存场地' : '创建场地'}</button>
          </div>
        </form>
      </section>
    </div>,
