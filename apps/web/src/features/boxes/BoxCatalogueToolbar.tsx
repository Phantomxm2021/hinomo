...
export function BoxCatalogueToolbar({ query, sort, onQueryChange, onSortChange }: BoxCatalogueToolbarProps) {
  return (
    <div className="grid gap-2 rounded-card border border-line bg-surface/75 p-2 sm:grid-cols-2">
      <input
        className="min-h-12 w-full rounded-control border border-line bg-surface px-3 text-ink placeholder:text-muted focus:border-brand focus:outline-none"
        type="search"
        aria-label="搜索箱子"
      ...
      />
      <select
        className="min-h-12 w-full rounded-control border border-line bg-surface px-3 text-ink focus:border-brand focus:outline-none"
        aria-label="箱子排序"
        value={sort}
