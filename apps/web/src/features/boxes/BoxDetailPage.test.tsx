...
test('offers retry and an explicit continue link when cached box data already has an error', async () => {
  ...
  const retry = await within(alert).findByRole('button', { name: '重试' })
  await waitFor(() => expect(retry).toBeEnabled())
  const callsBeforeRetry = mockGetBox.mock.calls.length
  mockGetBox.mockReturnValue(new Promise(() => undefined))
  ...
  expect(mockGetBox).toHaveBeenCalledTimes(callsBeforeRetry + 1)
