-- iOS WebKit does not reliably support Canvas WebP encoding. Keep Atlas
-- generation deterministic by signing and storing client Atlases as JPEG.

update public.packing_atlases
set object_key = pg_catalog.regexp_replace(object_key, '\.webp$', '.jpg')
where upload_status = 'pending' and object_key ~ '\.webp$';
...
is 'Creates an authenticated client Atlas JPEG upload for an active packing session.';
