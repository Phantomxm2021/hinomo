...
begin
  ...
  if detected.published_at is null then
    raise exception using errcode = '22023', message = 'detected item is not published';
  end if;
  if detected.analysis_revision <> session_row.current_revision then
    raise exception using errcode = '22023', message = 'detected item belongs to an outdated analysis revision';
  end if;
  if detected.review_status in ('dismissed'::public.packing_review_status, 'promoted'::public.packing_review_status) then
    raise exception using errcode = '22023', message = 'detected item has already been reviewed';
  end if;
  if detected.cover_object_key is null and detected.first_seen_photo_id is null
    and detected.representative_instance_id is null then
    raise exception using errcode = '22023', message = 'detected item has no source photo';
  end if;
