...
AI 项目是待审核的分析结果，不得直接插入箱子详情的正式“物品”清单，也不得在正式清单上方展开完整结果。箱子详情只保留一张紧凑的“AI 智能清单”状态入口卡，显示处理中、失败或识别项数；点击后在当前页面上方打开独立审核 Sheet。移动端使用接近全高的底部 Sheet，宽屏使用居中的有限宽度模态层。Sheet 内的 AI 项目可以沿用正式物品的缩略图与主信息行视觉语言，但必须保留“AI 识别”标识和审核操作。只有用户执行“设为正式物品”后，该项目才进入箱子详情的正式物品列表。
