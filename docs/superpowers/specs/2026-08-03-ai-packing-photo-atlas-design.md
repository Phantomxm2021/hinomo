...
每次拍照后立即回到取景状态，不显示名称、分类和数量表单。照片在本地先写入 IndexedDB，再进入受控上传队列，避免关闭 Sheet、页面刷新、弱网或短暂离线造成整次会话丢失。关闭 Sheet 只结束当前展示，不取消或删除会话；再次打开时恢复未上传草稿和服务端照片。

界面固定提示“每放入一个物件，拍一张”，拍摄完成后提示“继续记录下一件”。不得使用“每批”“一批物品”等可能让用户把多个独立物品合并拍摄的文案。

Web 端不得把所有设备伪装成原生相机：在无 hover 且使用粗指针的移动设备上，主操作显示“拍摄这件物品”，文件输入使用 `capture="environment"` 请求后置相机；在桌面或精细指针设备上，主操作显示“选择物品照片”，不设置 `capture`，由浏览器打开文件选择器。主操作位于内容区，不使用固定在底部的仿相机圆形快门。

已确认上传的照片以扑克牌式叠放展示，当前照片在最上层，前后相邻照片以轻微错位、旋转和缩放提示仍可浏览。默认定位到最新照片，新照片确认上传后自动切到最新一张。移动端支持左右滑动，桌面端同时支持鼠标拖动与上一张/下一张按钮，并始终显示“当前张数 / 总张数”。前端只渲染当前照片及前后各两张，避免长会话同时申请全部签名地址并解码大量图片。
...
- 最长边默认 2560 px；
- 优先 WebP，质量 85～90；
- 移动端直接拍照入口只声明接受 JPEG，要求 iPhone 在相机输出阶段完成系统级兼容转换，再由浏览器压缩为 WebP，避免在移动设备上使用高内存的 HEIC WASM 解码；相册或桌面文件选择器仍接受 HEIC/HEIF，Safari 等可原生解码的浏览器直接压缩为 WebP，其余浏览器按需加载 `heic-to` 的 libheif 解码器，先在本地转换为高质量 JPEG，再进入相同 WebP 压缩流程；原始 HEIC 不上传；
