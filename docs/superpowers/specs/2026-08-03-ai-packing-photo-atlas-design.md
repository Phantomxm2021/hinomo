...
每次拍照后立即回到取景状态，不显示名称、分类和数量表单。照片通常在本地先写入 IndexedDB，再进入受控上传队列，避免关闭 Sheet、页面刷新、弱网或短暂离线造成整次会话丢失。若 iOS 浏览器拒绝 IndexedDB Blob 事务，前端不得把它误报为压缩失败或阻断本次拍摄，而应将已压缩 JPEG 暂存在当前页面内存并直接上传；失败后允许在页面未关闭期间重试。关闭 Sheet 只结束当前展示，不取消或删除会话；IndexedDB 可用时，再次打开应恢复未上传草稿和服务端照片。
