...
| 00:26–00:30 | 08 | `<Picture 7>` → `<Picture 8>` | Attach exactly one QR label for the first time | Attach one QR label. |
| 00:30–00:34 | 09 | `<Picture 9>` → `<Picture 10>` | Bring the real scanner toward that same label | Scan the label later. |
...
- Preserve the exact supplied Nomo screen pixels and the fixed QR layer. If a generated clip distorts either region, re-composite the corresponding protected layer from `<Picture 3>`–`<Picture 6>` or `<Picture 8>`–`<Picture 10>` during editing. This correction does not create a new reference index.
