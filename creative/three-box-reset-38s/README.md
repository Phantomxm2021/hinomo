...
1. 先生成 Clip 01、02、07、08，检查物理动作：物品由手放入；短箱盖先合、长箱盖后合；合箱后的纸箱始终干净、没有标签或二维码。
2. 再生成 Clip 03–06，检查银色 iPhone 17 Pro Max 机身和 Nomo UI：不允许模型重写文字或发明控件。
3. 生成 Clip 09、10，检查纸箱上没有标签或二维码；最后直接切到 Box 1 的已保存库存。
4. 最后生成 Clip 11。CTA 只能做 2% 推镜，文字不得动画或重排。
5. 将合格片段按 `prompts/master-edit.md` 的顺序直接剪切组合，最终时长必须为 38.00 秒。

## 4. UI 锁层

H3 负责生成运动，不负责重新设计 Nomo 界面。如果模型发生文字漂移或图标变形，必须在剪辑阶段重新合成受保护层：
...
若手机发生轻微位移，用四角跟踪或 corner pin 把 `source/ui/` 对应截图重新贴回屏幕；若手机保持锁定，可直接覆盖。重新合成这些层不增加新的 Picture 索引。纸箱上不得添加二维码、Nomo Box 标签或任何其他贴纸。

## 5. 固定叙事逻辑

正确流程只有一条：把物品放入打开的 Box 1 → 用 Nomo 拍摄 → AI 得到三件物品 → 点击真实 `Add to list`，HDMI 行消失 → Box 1 显示三件已保存 → 人手按正确方向合箱 → 保持干净无标签的成箱画面 → 展示真实扫描器作为产品能力 → 直接显示 Box 1 已保存库存。

视频中没有搜索步骤、没有虚构的查看按钮、没有自动合箱，也没有二维码、标签或其他贴纸。
...
人工检查五项：UI 文字可读；Box 1 始终是同一个箱子；三件物品身份一致；合箱方向符合物理；纸箱全程没有二维码、标签或其他贴纸。
