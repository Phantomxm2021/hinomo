...
| `<Picture 8>` | `08-box-1-closed-labeled.png` | 前侧右方第一次出现完整 Nomo Box 横向标签 |
...
1. 先生成 Clip 01、02、07、08，检查物理动作：物品由手放入；短箱盖先合、长箱盖后合；完整横向 Nomo Box 标签只在 Clip 08 被贴到箱子前侧右方一次。
...
H3 负责生成运动，不负责重新设计 Nomo 界面、完整标签或二维码。如果模型发生文字漂移、图标变形、标签裁切或二维码变化，必须在剪辑阶段重新合成受保护层：
...
- Picture 8、10 的完整横向 Nomo Box 标签保护区域：`x=845, y=700, w=350, h=242`。

若手机发生轻微位移，用四角跟踪或 corner pin 把 `source/ui/` 对应截图重新贴回屏幕；若手机保持锁定，可直接覆盖。标签必须使用 Picture 8 的完整 `350 × 242` 透明保护层跟踪覆盖，不能只截取二维码，也不能重绘 `Nomo Box / BX-00038 / Space / Location / Scan to view box items`。重新合成这些层不增加新的 Picture 索引。

## 5. 固定叙事逻辑

正确流程只有一条：把物品放入打开的 Box 1 → 用 Nomo 拍摄 → AI 得到三件物品 → 点击真实 `Add to list`，HDMI 行消失 → Box 1 显示三件已保存 → 人手按正确方向合箱 → 第一次把完整横向 Nomo Box 标签贴在前侧右方 → 打开真实扫描器 → 扫标签左侧的二维码 → 直接显示 Box 1 库存。

视频中没有搜索步骤、没有虚构的查看按钮、没有自动合箱、没有预先存在的标签，也没有第二次贴标或独立的方形 QR-only 贴纸。
